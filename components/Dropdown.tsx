...
export const Dropdown: React.FC<DropdownProps> = ({
  ...
  const [activeColorPickerOption, setActiveColorPickerOption] = useState<string | null>(null);
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (dropdownRef.current && !dropdownRef.current.contains(event.target as Node)) {
        setIsOpen(false);
        setActiveColorPickerOption(null);
      }
    };
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div 
                        className={`w-6 h-6 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all hover:bg-black/5 ${style.text}`}
                        onClick={(e) => {
                          e.stopPropagation();
                          setActiveColorPickerOption(activeColorPickerOption === option ? null : option);
                        }}
                      >
                    ...
                    {activeColorPickerOption === option && (
                      <div 
                        className="absolute right-0 top-full mt-2 z-[110] bg-white border border-gray-100 shadow-xl shadow-black/[0.08] rounded-xl p-3 w-64 animate-in fade-in zoom-in-95 duration-200"
                        onClick={(e) => e.stopPropagation()}
                      >
                        <div className="grid grid-cols-5 gap-2">
                          {Object.entries(TAG_COLORS).map(([colorName, colorStyle]) => (
                            <Tooltip 
                              key={colorName}
                              content={colorName}
                            >
                              <div 
                                className={`w-8 h-8 rounded-lg cursor-pointer border-2 transition-all hover:scale-110 hover:shadow-md ${colorStyle.bg} ${colorName === colorKey ? 'border-primary-500 scale-110 shadow-sm' : colorStyle.border}`}
                                onClick={() => {
                                  if (onColorChange) {
                                    onColorChange(option, colorName);
                                  }
                                  setActiveColorPickerOption(null);
                                }}
                              />
