...
const TokenConfigDialog: React.FC<TokenConfigDialogProps> = ({ onClose }) => {
  ...
  const handleSave = () => {
    ...
    setManualToken('');
    toast.success('Token 已保存，您现在可以访问 API。');
    onClose();
    // Reload to apply token to pending requests might be needed, or just let user retry action
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-[80] flex items-center justify-center p-4 text-sm">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in-95 duration-200 flex flex-col">
        ...
        <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-white">
           <div>
             <h2 className="text-xl font-bold text-gray-800">API 鉴权配置</h2>
             <p className="text-sm text-gray-500 mt-1">配置 Console API Token 以访问后端服务。</p>
           </div>
        ...
        <div className="p-6 bg-gray-50">
           ...
           <div className="bg-primary-50 border border-primary-100 rounded-lg p-4 mb-6 flex gap-3 text-sm text-primary-800">
              ...
              <div>
                ...
                <p className="opacity-90">Token 将仅保存在您的本地浏览器缓存中 (localStorage)。</p>
              </div>
           </div>

           {/* Current Token Status */}
           <div className="mb-6">
               <label className="block text-xs font-semibold text-gray-500 mb-1.5 uppercase">当前状态</label>
               <div className="flex items-center justify-between bg-white border border-gray-200 rounded-lg p-3">
                   <div className="flex items-center gap-2">
                       <div className={`w-2.5 h-2.5 rounded-full ${savedToken ? 'bg-green-500' : 'bg-red-500'}`}></div>
                       <span className="font-medium text-gray-700">{savedToken ? '已配置 Token' : '未配置 Token'}</span>
                   </div>
                   {savedToken && (
           ...
           <div>
                ...
                <div className="flex justify-end gap-3">
                    ...
                    <button 
                        ...
                        className="bg-primary-600 text-white px-5 py-2 rounded-lg text-sm font-bold hover:bg-primary-700 disabled:opacity-50 disabled:cursor-not-allowed shadow-md transition-all active:scale-95"
                    >
                        保存并重载
                    </button>
                </div>
