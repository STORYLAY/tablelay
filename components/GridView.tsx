...
import { Column, Row, Table, FieldType, ColorRule, RowHeight, GroupCondition } from '../types';
import { ICONS, FIELD_TYPE_ICONS, getTagColor, formatDateForInput, formatDateForDisplay, parseLinkValues, parseJsonArray, formatFieldValue } from '../constants';
...
interface GridViewProps {
  ...
  onAddRow: (initialData?: Record<string, any>, specificId?: string, index?: number) => void;
        ...
        >
            <div ref={listRef} className="max-h-60 overflow-y-auto custom-scrollbar py-1">
                {loading ? <div className="p-2 text-center text-gray-400 text-xs">加载中...</div> : (
                    depts.length > 0 ? depts.map((d, idx) => {
...
const GridView: React.FC<GridViewProps> = ({
  ...
  const focusedCellAtMouseDown = useRef<{ rowId: string, colId: string } | null>(null);
  const hiddenInputRef = useRef<HTMLTextAreaElement>(null);
  const isComposingRef = useRef(false);
  const [dragFillStart, setDragFillStart] = useState<{ rowId: string, colId: string, rowIndex: number, colIndex: number } | null>(null);
  const [dragFillEnd, setDragFillEnd] = useState<{ rowId: string, colId: string, rowIndex: number, colIndex: number } | null>(null);
  ...
  const [selectionEnd, setSelectionEnd] = useState<{rowId: string, colId: string} | null>(null);
  ...
  const [dropTargetColIndex, setDropTargetColIndex] = useState<number | null>(null);

  const visibleRows = useMemo(() => getVisibleRows(rows, expandedRowIds), [rows, expandedRowIds]);
  ...
  const selectionRange = useMemo(() => {
          ...
          if (focusedCell) {
              ...
              if (rIdx !== -1 && cIdx !== -1) {
                  return {
                      ...
                      minR: rIdx, maxR: rIdx, minC: cIdx, maxC: cIdx
                  };
              }
          }
          return null;
      ...
      if (startR === -1 || endR === -1 || startC === -1 || endC === -1) return null;
              ...
              for (let c = minC; c <= maxC; c++) {
                  ...
                  if (val === null || val === undefined) val = '';
                      ...
                      if (Array.isArray(val)) {
                          val = val.map(v => typeof v === 'object' ? (v.name || v.id || JSON.stringify(v)) : v).join(', ');
                      } else {
        ...
        rowsData.forEach((rowData, rIdx) => {
            ...
            if (!targetRow) {
                ...
                rowData.forEach((cellValue, cIdx) => {
                    ...
                    if (activeEditingCell && activeEditingCell.rowId === targetRow.id && activeEditingCell.colId === targetCol.id) {
  ...
  }, [focusedCell, columns, visibleRows, onCellChange, onBatchProcessRows, onDirectAddRow, onAddRow]);
  const [activeEditingCell, setActiveEditingCell] = useState<{ rowId: string, colId: string, rect?: DOMRect } | null>(null);
  const handleSetActiveEditingCell = (cell: { rowId: string, colId: string, rect?: DOMRect } | null) => {
      if (cell) {
  ...
  const handleColDrop = (e: React.DragEvent) => {
    e.preventDefault();
    if (draggedColIndex === null || dropTargetColIndex === null || draggedColIndex === dropTargetColIndex) {
    ...
    const draggedAllIndex = newAllColumns.findIndex(c => c.id === draggedCol.id);
    const targetAllIndexOriginal = newAllColumns.findIndex(c => c.id === targetCol.id);
  ...
  const startEditing = React.useCallback((rowId: string, colId: string, val: any) => {
      if (hoverTimeoutRef.current) clearTimeout(hoverTimeoutRef.current);
      setHoveredCell(null); // Clear hover when editing starts
      handleSetActiveEditingCell({ rowId, colId, rect: document.querySelector(`[data-row-id="${rowId}"][data-col-id="${colId}"]`)?.getBoundingClientRect() });
      setEditingValue(val);
  }, [handleSetActiveEditingCell]);

  const onCellChangeInternal = React.useCallback((rowId: string, colId: string, value: any) => {
      onCellChange(rowId, colId, value);
      if (activeEditingCell && activeEditingCell.rowId === rowId && activeEditingCell.colId === colId) {
  ...
  const navigateToCell = React.useCallback((rowId: string, colId: string, direction: 'next' | 'prev') => {
      ...
      if (nextRow && nextCol) {
          ...
          setTimeout(() => {
              const element = document.querySelector(`[data-row-id="${nextRow.id}"][data-col-id="${nextCol.id}"]`);
              if (element) {
  ...
  const moveVertical = React.useCallback((rowId: string, colId: string, offset: number) => {
      ...
      if (nextR >= 0 && nextR < visibleRows.length) {
          ...
          // Ensure the cell is visible
          setTimeout(() => {
              const element = document.querySelector(`[data-row-id="${nextRow.id}"][data-col-id="${colId}"]`);
              if (element) {
                  element.scrollIntoView({ block: 'nearest', inline: 'nearest' });
              }
          }, 0);
      ...
      const handleKeyDown = (e: KeyboardEvent) => {
          ...
          if (e.key === 'F2') {
              ...
              if ([FieldType.USER, FieldType.DEPARTMENT, FieldType.SELECT, FieldType.MULTI_SELECT, FieldType.ATTACHMENT].includes(col.type)) {
                  ...
                  handleSetActiveEditingCell({ rowId: focusedCell.rowId, colId: focusedCell.colId, rect });
          ...
          if (e.key === ' ') {
              if (col.type === FieldType.CHECKBOX) {
                  e.preventDefault();
                  onCellChangeInternal(focusedCell.rowId, focusedCell.colId, !row.data[col.id]);
              }
              return;
          }

          if (e.key === 'Delete' || e.key === 'Backspace') {
              const editableTypes = [FieldType.TEXT, FieldType.NUMBER, FieldType.HYPERLINK, FieldType.SELECT, FieldType.MULTI_SELECT, FieldType.USER, FieldType.DEPARTMENT, FieldType.DATE, FieldType.LINK, FieldType.ATTACHMENT, FieldType.CHECKBOX];
              if (editableTypes.includes(col.type)) {
                  e.preventDefault();
                  onCellChangeInternal(focusedCell.rowId, focusedCell.colId, col.type === FieldType.CHECKBOX ? false : null);
              }
              return;
          }
          ...
          if (e.key.length === 1) {
              ...
              if (textTypes.includes(col.type)) {
                  ...
                  const choiceTypes = [FieldType.SELECT, FieldType.MULTI_SELECT, FieldType.USER, FieldType.DEPARTMENT, FieldType.DATE, FieldType.LINK, FieldType.ATTACHMENT];
  ...
  }, [focusedCell, activeEditingCell, columns, rows, visibleRows, navigateToCell, moveVertical, saveEditing, handleSetActiveEditingCell, onCellChangeInternal, startEditing]);
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
        if (resizingCol) {
            ...
            onColumnResize(resizingCol.id, Math.max(minWidth, resizingCol.startWidth + diff));
        }
        if (resizingRow) {
            setIsDragging(true);
            const diff = e.clientY - resizingRow.startY;
            onRowHeightChange(resizingRow.id, Math.max(20, resizingRow.startHeight + diff));
        }
    };
    const handleMouseUp = () => {
  ...
  const handleDragFillStart = (e: React.MouseEvent, rowId: string, colId: string) => {
    e.stopPropagation();
    e.preventDefault();
  ...
  const handleDragFillEnd = () => {
    ...
    for (let r = minR; r <= maxR; r++) {
        for (let c = minC; c <= maxC; c++) {
            ...
            if (targetCol && (targetCol.type === FieldType.LOOKUP || targetCol.type === FieldType.SEARCH_REFERENCE || targetCol.type === FieldType.FORMULA)) {
                continue;
            }
  ...
  const handleAddSubRowInternal = (rowId: string, initialData?: Record<string, any>) => {
  ...
  const getRowColorClass = (row: Row) => {
      ...
      for (const rule of colorRules) {
          ...
          if ((col?.type === FieldType.USER || col?.type === FieldType.DEPARTMENT) && typeof val === 'string' && val.startsWith('[')) {
              try {
                  val = JSON.parse(val);
              } catch (e) {}
          }
          ...
          if ((col?.type === FieldType.USER || col?.type === FieldType.DEPARTMENT) && typeof ruleVal === 'string' && ruleVal.startsWith('[')) {
              try {
          ...
          if ((col?.type === FieldType.USER || col?.type === FieldType.DEPARTMENT) && Array.isArray(ruleVal)) {
              const selectedIds = ruleVal.map(v => String(v && typeof v === 'object' ? v.id : v));
              if (selectedIds.length === 0) {
                  if (rule.operator === 'is_empty' || rule.operator === 'isEmpty') match = !val || (Array.isArray(val) && val.length === 0);
                  else if (rule.operator === 'is_not_empty' || rule.operator === 'isNotEmpty') match = !!val && (!Array.isArray(val) || val.length > 0);
                  else continue;
              } else {
                  const rowIds = Array.isArray(val) 
                      ? val.map(v => String(v && typeof v === 'object' ? v.id : v)) 
                      : (val ? [String(val.id || val)] : []);

                  const hasMatch = rowIds.some(id => selectedIds.includes(id));
                  
                  switch (rule.operator) {
                      case 'is':
                      case 'eq': match = hasMatch; break;
                      case 'isNot':
                      case 'neq': match = !hasMatch; break;
                      case 'contains': match = hasMatch; break;
                      case 'not_contains': match = !hasMatch; break;
                      case 'isEmpty':
                      case 'is_empty': match = rowIds.length === 0; break;
                      case 'isNotEmpty':
                      case 'is_not_empty': match = rowIds.length > 0; break;
                  }
              }
          } else {
                  ...
                  if (Array.isArray(v)) {
                      return v.map(item => {
                          if (typeof item === 'object' && item !== null) {
                              return item.real_name || item.name || item.dept_name || item.id || String(item);
                          }
                          return String(item);
                      }).filter(Boolean).join(', ');
                  }
              ...
              switch (rule.operator) {
                  case 'is':
                  case 'eq': match = comparableVal === ruleComparableValue; break;
                  case 'isNot':
                  case 'neq': match = comparableVal !== ruleComparableValue; break;
                  case 'contains': match = comparableVal.toLowerCase().includes(ruleComparableValue.toLowerCase()); break;
                  case 'not_contains': match = !comparableVal.toLowerCase().includes(ruleComparableValue.toLowerCase()); break;
                  case 'isEmpty':
                  case 'is_empty': match = !comparableVal; break;
                  case 'isNotEmpty':
                  case 'is_not_empty': match = !!comparableVal; break;
              }
          }
  ...
  const handleRowMove = async (rowId: string, targetRowId: string) => {
      try {
          ...
          const targetSiblings = allRealRows.filter(r => normalizeParentId(r.parent_id) === targetParentId);

          let targetIndex = targetSiblings.findIndex(r => r.id === targetRowId);
          
          if (targetParentId === sourceParentId) {
              const sourceIndex = targetSiblings.findIndex(r => r.id === rowId);
  ...
  const formatNumber = (val: any, format: string): string => {
      ...
      switch (format) {
          case '0': return num.toFixed(0);
          case '0.0': return num.toFixed(1);
  ...
  const renderCell = (row: Row, col: Column, level: number = 0, parentLines: boolean[] = []) => {
      const val = row.data[col.id];
      ...
      const TreeLines = () => {
          ...
          return (
              <div className="absolute left-0 top-0 bottom-0 flex pointer-events-none" style={{ width: level * 24 }}>
                  {parentLines.map((hasMore, i) => (
                      <div key={i} className="relative w-6 h-full flex justify-center shrink-0">
                          ...
                          {i === level - 1 && (
                              <>
                                  <div className={`absolute top-0 ${parentLines[i] ? 'bottom-0' : 'h-1/2'} left-1/2 w-[1px] border-l border-dashed border-gray-300`}></div>
      ...
      if (isEditing) {
          if (col.type === FieldType.LOOKUP || col.type === FieldType.SEARCH_REFERENCE || col.type === FieldType.FORMULA) {
          ...
          if (col.type === FieldType.USER) {
              return (
                  <ClickOutsideWrapper onClickOutside={() => handleSetActiveEditingCell(null)} className="w-full h-full relative">
                      ...
                      <UserSelector 
                          value={val} 
                          onChange={(newVal) => onCellChangeInternal(row.id, col.id, newVal)}
                          onClose={() => handleSetActiveEditingCell(null)}
                      />
                  </ClickOutsideWrapper>
              );
          }
          ...
          if (col.type === FieldType.DEPARTMENT) {
              return (
                  <ClickOutsideWrapper onClickOutside={() => handleSetActiveEditingCell(null)} className="w-full h-full relative">
                      <div className="w-full h-full flex items-center px-2 gap-1 overflow-hidden border-2 border-primary-500 bg-white absolute inset-0 z-[101]">
                          {(Array.isArray(val) ? val : (val ? [val] : [])).map((d: any, i: number) => (
                          ...
                          {(Array.isArray(val) ? val : (val ? [val] : [])).length === 0 && <span className="text-gray-300 text-xs">选择部门</span>}
                      </div>
                      <DepartmentSelector 
                          value={val} 
                          onChange={(newVal) => onCellChangeInternal(row.id, col.id, newVal)}
              ...
              return (
                  <div className="w-full h-full relative">
                      <div className="w-full h-full flex items-center px-2 border-2 border-primary-500 bg-white absolute inset-0 z-[101]">
                          {val ? (
                              <div className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs ${colorStyle.bg} ${colorStyle.text} border border-transparent`}>
                                  {val}
                              </div>
                          ) : null}
                      </div>
                          ...
                          {files.length > 0 && (
                              <div className="flex items-center gap-1 overflow-hidden">
                                  {files.map((f: any, i: number) => (
                                      ...
                                      >
                                          <span className="text-[10px] font-bold uppercase">{String(f.extension || f.name?.split('.').pop() || 'F').substring(0, 3)}</span>
                                      </div>
                                  ))}
                              </div>
                          )}
                                          ...
                                          (f.extension && ['jpg', 'jpeg', 'png', 'gif', 'webp'].includes(f.extension.toLowerCase())) ? (
                                              <>
                                                  ...
                                                  <div className="hidden flex-col items-center justify-center text-gray-400 w-full h-full">
                                                      ...
                                                      <span className="text-xs font-medium uppercase truncate px-1 w-full text-center">{f.extension || f.name?.split('.').pop() || 'FILE'}</span>
                                                  </div>
                                              </>
                                          ) : (
                                              <div className="flex flex-col items-center justify-center text-gray-400">
                                                  <ICONS.File className="w-8 h-8 mb-1 text-primary-500" />
                                                  <span className="text-xs font-medium uppercase">{f.extension || f.name?.split('.').pop() || 'FILE'}</span>
                                              </div>
                                          )
                                      ) : (
                                          <div className="flex flex-col items-center justify-center text-gray-400">
                                      ...
                                      <Tooltip content="删除">
                                        <button 
                                            ...
                                            onClick={(e) => {
                                                ...
                                                newFiles.splice(i, 1);
                                                onCellChangeInternal(row.id, col.id, newFiles);
                                            }}
                                        >
                                  ...
                                  onChange={async (e) => {
                                      if (e.target.files && e.target.files.length > 0) {
                                          try {
                                              ...
                                              onCellChangeInternal(row.id, col.id, [...files, ...newFiles]);
                                          } catch (err) {
      ...
      if (col.type === FieldType.TEXT) {
          ...
          let displayValue = (editingValue !== null && editingValue !== undefined) ? editingValue : (val || '');
          if (col.type === FieldType.DATE) {
              displayValue = formatDateForInput(displayValue, inputType === 'datetime-local');
          }

          const handleSave = () => {
             let valToSave = editingValue;
             if (inputType === 'datetime-local' && valToSave && typeof valToSave === 'string') {
                 valToSave = valToSave.replace('T', ' ');
             }
             if (activeEditingCell && !isSavingRef.current) {
                 isSavingRef.current = true;
                 onCellChangeInternal(activeEditingCell.rowId, activeEditingCell.colId, valToSave);
                 handleSetActiveEditingCell(null);
                 setTimeout(() => { isSavingRef.current = false; }, 100);
             }
          };
          ...
          return (
              ...
              >
                  {depts.length > 0 ? depts.map((d: any, i: number) => (
                      <div key={i} className="flex items-center gap-1 bg-gray-100 text-gray-700 px-1.5 py-0.5 rounded text-[10px] border border-gray-200 shrink-0">
                          <ICONS.Building className="w-3 h-3 text-gray-500" />
                          <span className="truncate max-w-[60px]">{highlightText(d.name)}</span>
                      </div>
      ...
      if (col.type === FieldType.SELECT) {
          ...
          return (
              ...
              >
                  {val ? (
                      <span className={`px-2 py-0.5 rounded text-xs ${colorStyle.bg} ${colorStyle.text} truncate max-w-full`}>
      ...
      if (col.type === FieldType.MULTI_SELECT) {
          ...
          return (
              <div 
                  ...
                  onMouseLeave={handleCellMouseLeave}
              >
                  {values.length > 0 ? values.map((v: string, idx: number) => {
                      const colorStyle = getTagColor(v, col.config?.option_colors);
                      return (
                          <span key={idx} className={`px-1.5 py-0.5 rounded text-[10px] whitespace-nowrap ${colorStyle.bg} ${colorStyle.text}`}>
                              {highlightText(v)}
                          </span>
                      );
                  }) : <span className="text-gray-300 text-xs">请选择</span>}
                  ...
                  {files.length > 0 && (
                      <div className="flex items-center gap-1 overflow-hidden">
                          {files.map((f: any, i: number) => {
                              ...
                              const ext = String(f.extension || filename?.split('.').pop() || 'FILE').toUpperCase();
                              return (
                                  <div 
                                      key={i} 
                                      className="flex items-center justify-center w-6 h-6 bg-primary-50 text-primary-600 rounded shrink-0 border border-primary-100"
                                  ...
                                  >
                                      <span className="text-[8px] font-bold">{ext.substring(0, 3)}</span>
                                  </div>
                              );
                          })}
                      </div>
      ...
      if (col.type === FieldType.NUMBER) {
          return (
            <div 
                ...
                onMouseLeave={handleCellMouseLeave}
            >
                {highlightText(formatNumber(val, col.config?.format || col.format || ''))}
                <CommentBadge count={commentCount} rowId={row.id} colId={col.id} />
            </div>
          );
      }
      ...
      if (col.type === FieldType.DATE) {
        return (
          <div 
              ...
              onMouseLeave={handleCellMouseLeave}
          >
              {highlightText(formatDateForDisplay(val, col.config?.format || col.format || 'YYYY-MM-DD'))}
              <CommentBadge count={commentCount} rowId={row.id} colId={col.id} />
          </div>
        );
      }

      if (col.type === FieldType.LOOKUP || col.type === FieldType.SEARCH_REFERENCE) {
          // These are derived fields, usually read-only
          // They might contain multiple values if it's a search reference
          const displayVal = Array.isArray(val) ? val.map(v => typeof v === 'object' ? (v.name || v.id) : String(v)).join(', ') : String(val || '');
          return (
            <div 
            ...
            >
                <div className="flex items-center gap-1 text-gray-500 bg-gray-50 px-1.5 py-0.5 rounded border border-gray-100 w-fit max-w-full">
                    <span className="font-mono font-bold text-[10px] text-gray-400 shrink-0">🔍</span>
                    <span className="truncate font-mono text-gray-700 text-xs">{highlightText(String(displayVal !== undefined && displayVal !== null ? displayVal : ''))}</span>
                </div>
                <CommentBadge count={commentCount} rowId={row.id} colId={col.id} />
            </div>
          );
      ...
      if (col.type === FieldType.FORMULA) {
          ...
          const isNumeric = !isNaN(Number(displayVal)) && displayVal !== '' && displayVal !== null && displayVal !== undefined && typeof displayVal !== 'boolean';
          ...
          return (
            ...
            >
                ...
                {highlightText(String(displayVal !== undefined && displayVal !== null ? displayVal : ''))}
                <CommentBadge count={commentCount} rowId={row.id} colId={col.id} />
            </div>
          );
      }
      ...
      return (
        ...
        >
            ...
            <span className="truncate flex-1">{highlightText(String(val || ''))}</span>
            ...
            {columns[0].id === col.id && (
                <>
                    ...
                    <div className="hidden group-hover/cell:flex items-center gap-0.5 bg-white border border-gray-200/80 p-0.5 rounded-md absolute right-2 top-1/2 -translate-y-1/2 shadow-sm z-20 whitespace-nowrap">
                        ...
                        <CommentBadge count={commentCount} rowId={row.id} colId={col.id} isHover />
                    </div>
    ...
    const renderRowRecursive = (row: Row, level = 0, indexPos: number, parentLines: boolean[] = []): React.ReactNode => {
        ...
        if (row.isGroup) {
            return (
                <React.Fragment key={row.id}>
                    ...
                    >
                        <div className="flex items-center px-4 w-full text-sm font-medium text-gray-700" style={{ paddingLeft: `${level * 24 + 16}px` }}>
                            <div className="w-4 h-4 mr-2 flex items-center justify-center shrink-0">
                                {isExpanded ? <ICONS.ChevronDown className="w-4 h-4 text-gray-500" /> : <ICONS.ChevronRight className="w-4 h-4 text-gray-500" />}
                            </div>
                            <div className="flex items-center gap-2">
                                {((row.groupKey && !Array.isArray(row.groupKey)) ? (row.groupKey as any).label : (row.groupKey as any[] || [])).map((k: any, i: number) => (
                                    <React.Fragment key={i}>
                                        {i > 0 && <span className="text-gray-400">/</span>}
                                        <span className={!k || k === '(空)' ? 'text-gray-400 italic' : ''}>{k || '(空)'}</span>
                                    </React.Fragment>
                                ))}
                                <span className="text-gray-400 text-xs ml-2 font-normal">{row.data.count} 条记录</span>
                            </div>
                        </div>
                    </div>
                    {isExpanded && row.children && (
                        <>
                            {row.children.map((child, idx) => renderRowRecursive(child, level + 1, idx, [...parentLines, idx < row.children!.length - 1]))}
                            <div 
                                ...
                                style={{ height: 36, paddingLeft: `${(level + 1) * 24 + 16 + 24}px` }}
                                onClick={() => {
                                  const initialData: Record<string, any> = {};
                                  if (groups && row.groupKey) {
                                      groups.forEach((g, i) => {
                                          const val = Array.isArray(row.groupKey) ? row.groupKey[i] : (row.groupKey as any)?.value?.[i];
                                          if (val && val !== '(空)') {
          ...
          <React.Fragment key={row.id}>
              ...
              >
                  ...
                  >
                     ...
                     {!isSelected && (
                         <>
                             <span className="text-gray-400 group-hover/handle:hidden ml-1">{indexPos + 1}</span>
                             <div className="hidden group-hover/handle:block ml-3.5"><CheckboxUnchecked /></div>
                         </>
                     )}
                  ...
                  {columns.map((col, index) => {
                      ...
                      const isInDragRange = dragRange?.rowIds.has(row.id) && dragRange?.colIds.has(col.id);
                      const isInSelectionRange = selectionRange?.rowIds.has(row.id) && selectionRange?.colIds.has(col.id);
                  ...
                  <div 
                     ...
                     onMouseDown={(e) => {
                         e.stopPropagation();
                         setResizingRow({ id: row.id, startY: e.clientY, startHeight: rowHeightVal });
                     }}
                  />
              </div>
              {isExpanded && row.children && row.children.map((child, idx) => renderRowRecursive(child, level + 1, idx, [...parentLines, idx < row.children.length - 1]))}
          </React.Fragment>
      );
  };
    ...
    >
       ...
       <div className="flex-1 overflow-auto bg-white" onScroll={handleScroll}>
           <div className="w-max min-w-full">
               {/* Header */}
               <div className="flex border-b border-gray-200 bg-gray-50 font-medium text-xs text-gray-500 sticky top-0 z-40">
                  <div 
                      className="w-10 shrink-0 border-r border-gray-200 flex items-center justify-center bg-gray-100 cursor-pointer"
               ...
               {rows.slice(0, visibleRowCount).map((row, idx) => renderRowRecursive(row, 0, idx, []))}
       ...
       {contextMenu && (() => {
            const isMultiCellSelection = selectionRange && 
                ...
                (contextMenu.colId ? selectionRange.colIds.has(contextMenu.colId) : true) && 
            ...
            >
                    ...
                    <>
                        ...
                        {contextMenu.colId && (
                            <>
                             ...
                             onCellChangeInternal(contextMenu.rowId, contextMenu.colId, copiedCellData);
                                            setContextMenu(null);
                                        }
                                    }}
                                >
                        ...
                        >
                            <div className="flex items-center gap-2 cursor-pointer" onClick={() => {
                                onInsertRow(contextMenu.rowId, 'before', undefined, Number(insertAboveCount) || 1);
                                setContextMenu(null);
                            }}>
                            ...
                            <input 
                                ...
                                onChange={(e) => {
                                    ...
                                    if (val === '') setInsertAboveCount('');
                                        ...
                                        if (!isNaN(num)) setInsertAboveCount(Math.max(1, num));
                                    }
                                }}
                                onClick={(e) => e.stopPropagation()}
                            />
                        ...
                        >
                            <div className="flex items-center gap-2 cursor-pointer" onClick={() => {
                                onInsertRow(contextMenu.rowId, 'after', undefined, Number(insertBelowCount) || 1);
                                setContextMenu(null);
                            }}>
                            ...
                            <input 
                                ...
                                onChange={(e) => {
                                    ...
                                    if (val === '') setInsertBelowCount('');
                                        ...
                                        const num = parseInt(val);
                                        if (!isNaN(num)) setInsertBelowCount(Math.max(1, num));
                                    }
                                }}
                                onClick={(e) => e.stopPropagation()}
                            />
                        ...
                        {columns.length > 0 && columns[0].id === contextMenu.colId && (
                            <div 
                                className="px-4 py-2 hover:bg-gray-50 cursor-pointer flex items-center gap-2 text-gray-700"
                                onClick={() => {
                        ...
                        <div 
                            ...
                            onClick={() => {
                                if (columns.length > 0) {
                                    onOpenComment(contextMenu.rowId, contextMenu.colId || columns[0].id);
                                }
                                setContextMenu(null);
                            }}
                        >
                ...
                <div 
                    ...
                    onClick={() => {
                        ...
                        if (selectionRange && selectionRange.rowIds.has(contextMenu.rowId)) {
       ...
       <div className="flex items-center px-4 border-t border-gray-200 bg-white text-[11px] text-gray-500 shrink-0 select-text z-40 relative mt-auto h-8 shadow-sm">
           共 <span className="mx-1 font-semibold text-gray-700">{totalCount !== undefined ? totalCount : rows.length}</span> 条记录
       </div>
